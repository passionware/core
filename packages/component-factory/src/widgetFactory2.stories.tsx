...
import { widgetBuilder } from "./widgeFactory";
...
function createPaymentWidget(config: WidgetConfig) {
  return function PaymentWidget(props: WidgetOwnProps) {
...
const MockPaymentWidget = widgetBuilder
  .takeProps<WidgetInjectableProps & WidgetOwnProps>()
  .passProps(({ className }) => ({ className }))
  .build((useProps) =>
    createPaymentWidget({
      useProduct: (productId) => {
        const { userId, shouldFail } = useProps();
        return testQuery.useData(
          testQuery.of(
            useMemo(
              () =>
                shouldFail
